  ...
  return (
    <>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-lg border border-brand-navy/10 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold tracking-normal text-brand-navy">Paquetes por estado</h2>
              <p className="mt-1 text-sm text-brand-muted">Distribución actual de la operación.</p>
            </div>
          ...
          <div className="mt-5 h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={packageStatusData}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgb(var(--brand-navy-rgb) / 12%)" />
                <XAxis dataKey="status" tick={{ fontSize: 11 }} interval={0} height={60} />
                <YAxis allowDecimals={false} tick={{ fontSize: 12 }} />
                <Tooltip />
                <Bar dataKey="total" fill="var(--color-brand-red)" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>

        <div className="rounded-lg border border-brand-navy/10 bg-brand-navy p-5 text-white shadow-sm">
          <p className="text-sm font-semibold text-white/75">Operación de hoy</p>
          ...
          <div className="mt-6 space-y-4">
            <div className="rounded-lg bg-white/10 p-4">
              ...
              <p className="mt-1 text-3xl font-semibold text-white/75">{pendingQuotations}</p>
            </div>
            <div className="rounded-lg bg-white/10 p-4">
              <p className="text-sm font-semibold">Último paquete creado</p>
              <p className="mt-1 text-sm text-white/70">{packages[0]?.packageCode ?? 'Sin paquetes'}</p>
            </div>
            <div className="rounded-lg bg-white/10 p-4">
              <p className="text-sm font-semibold">Fecha operativa</p>
              <p className="mt-1 text-sm text-white/70">{formatDate(new Date().toISOString())}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <div>
          <h2 className="mb-3 text-lg font-semibold tracking-normal text-brand-navy">Últimos pedidos</h2>
          <DataTable data={orders.slice(0, 5)} columns={orderColumns} getRowKey={(order) => order.id} />
        </div>
        <div>
          <h2 className="mb-3 text-lg font-semibold tracking-normal text-brand-navy">Últimos paquetes</h2>
